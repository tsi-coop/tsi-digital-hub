...
import org.tsicoop.digitalhub.app.Notification;
...
public class InterceptingFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (apiRegistry.containsKey(servletPath.trim())) {
            ...
            if (!(uriIdentifier.equalsIgnoreCase(ADMIN_URI)
                    ||uriIdentifier.equalsIgnoreCase(APP_URI)
                    ||uriIdentifier.equalsIgnoreCase("policy")
                    ||uriIdentifier.equalsIgnoreCase("consent")
                    ||uriIdentifier.equalsIgnoreCase("principal"))){
                res.sendError(400);
                return;
            }
            ...
            // Check
             try {
                 if(!servletPath.contains("api/app/login")
                         ...
                         && !servletPath.contains("api/policy")
                         && !servletPath.contains("api/consent")
                         && !servletPath.contains("api/principal")) {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        System.out.println("TSI Coop Service started in "+System.getenv("TSI_DIGITAL_HUB_ENV")+" environment");
    }
}
