...
public class InputProcessor {
    ...
    public static String getClientIpAddress(HttpServletRequest request) {
        ...
        if (ipAddress == null || ipAddress.isEmpty() || "unknown".equalsIgnoreCase(ipAddress)) {
            ipAddress = request.getHeader("X-Real-IP"); // Check X-Real-IP if Nginx also sets it
        }
        if (ipAddress == null || ipAddress.isEmpty() || "unknown".equalsIgnoreCase(ipAddress)) {
            ipAddress = request.getRemoteAddr(); // Fallback to direct remote address
        }

        // If X-Forwarded-For contains a comma-separated list, the first IP is the client's.
