package org.tsicoop.digitalhub.batch;

import org.postgresql.core.BaseConnection;
import org.postgresql.copy.CopyManager;
import org.tsicoop.digitalhub.framework.BatchDB;
...
public class DBSync{
    ...
    public static void main(String[] args) {
        ...
        if(args.length != 0){
            ...
            projectdir = "C:/work/tsi-coop";
        }
        try {
            String configpath = projectdir+"/db/_config.tsi";//args[0]
            FileInputStream configFileReader = new FileInputStream(configpath);
            configProps.load(configFileReader);
            String schemapath = projectdir+"/db/_schema.tsi";
            ...
            String datadir = projectdir+"/db";
            sync.loadMasterData(configProps, datadir, "_state");
            sync.loadMasterData(configProps, datadir, "_city");
            sync.loadMasterData(configProps, datadir, "_taxonomy");
            sync.loadMasterData(configProps, datadir, "_solution_tags");
            sync.loadMasterData(configProps, datadir, "_service_tags");
            sync.loadMasterData(configProps, datadir, "_training_tags");
            sync.loadMasterData(configProps, datadir, "_skill_tags");
            sync.loadMasterData(configProps, datadir, "_lookup");
            sync.loadMasterData(configProps, datadir, "_role");
