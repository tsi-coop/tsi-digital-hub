...
const TSIAddSolutionModal = ({
    ...
    const [openSolutionTags, setOpenSolutionTags] = useState<any>([])
    ...
    const getViewSolutionsData = (id: any) => {
        ...
        apiInstance.viewSolutions(body)
            .then((response: any) => {
                if (response?.data) {
                    setSolutionData(response?.data)
                    setSolutionTitle(response?.data?.title)
                    setSolutionLink(response?.data?.solution_link)
                    setPositioning(response?.data?.positioning)
                    setKeyFeatures(response?.data?.features)
                    ...
                    // }
                    // console.log("selectedIndustryselectedIndustry", selectedIndustry)
                    setSelectedSkills(response?.data?.skills_used?.replace(/[{}]/g, '').split(',') || [])
                    setSelectedSolutions(response?.data?.solutions_offered?.replace(/[{}]/g, '').split(',') || [])
                }
                setLoad(false)
            })
            .catch((error: any) => {
    ...
    React.useEffect(() => {
        ...
        getSkills()
        if (edit) {
            getViewSolutionsData(editablePost?.id)
        }
    }, []);


    useEffect(() => {
        if (solutionData?.industry && indVerticles?.length > 0) {
            const selected = indVerticles.find((item: any) => item.key === solutionData?.industry);
            if (selected) {
                setIndustry({ key: selected.key, value: selected.value });
            }
        }
    }, [solutionData?.industry, indVerticles])


    const addSolutionsData = () => {
        setLoad(true)
        ...
        const editbody = {
            ...
            "solutions_offered": selectedSolutions,
        ...
        apiInstance.addSolutions(edit ? editbody : body)
            .then((response: any) => {
                if (response?.data?._added) {
                    ...
                    setSnackbar({
                        open: true,
                        severity: 'success',
                        message: "Solutions post added successful",
                    })
                    setTimeout(() => {
                        setIsOpen(false)
                        onSubmit()
                    }, 1000)
                } else if (response?.data?._added == false) {
                    setSnackbar({
                        open: true,
                        severity: 'error',
                        message: "Unable to add the solution",
                    })
                    ...
                    setSnackbar({
                        open: true,
                        severity: 'success',
                        message: "Solutions post edited successful"
                    })
                    setTimeout(() => {
                        setIsOpen(false)
                        onSubmit()
                ...
                {(!load) ? (<div style={{ width: '100%', height: "100%" }}>
                    ...
                    >
                        <span style={{
                            ...
                            {!preview ? (edit ? `Edit ${title}` : `Add ${title}`) : "Preview Solution"}
                        </span>
                    ...
                    {(!preview) && (
                        ...
                        >
                            <TSITextfield
                                ...
                                multiline={false}
                                rows={1}
                                handleChange={(event: any) => { setSolutionTitle(event.target.value) }}
                            />
                            ...
                            <TSITextfield
                                ...
                                multiline={false}
                                rows={1}
                                handleChange={(event: any) => { setSolutionLink(event.target.value) }}
                            />
                            ...
                            {(industryData && skillData && industry.key) && (<div
                                style={{
                                    display: 'flex',
                                    flexDirection: 'column',
                                    alignItems: "flex-start",
                                    ...
                                    >
                                        ...
                                        <div style={{ display: 'flex', flexDirection: "column", justifyContent: "flex-start", alignItems: "flex-start", width: "100%", }}>
                                            ...
                                            <ListItem sx={{
                                                ...
                                                borderBottomLeftRadius: openSolutionTags ? "0px" : "5px",
                                                borderBottomRightRadius: openSolutionTags ? "0px" : "5px",
                                                width: '100%',
                                                paddingTop: "0px",
                                                paddingBottom: '0px',
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-strat', alignItems: "flex-start", width: '100%', }}>
                                                    <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: "center", width: '100%', }}>
                                                        ...
                                                        <IconButton onClick={() => { setOpenSolutionTags(!openSolutionTags) }}>
                                                            {openSolutionTags ? <ExpandLess /> : <ExpandMore />}
                                                        </IconButton>
                                                    </div>

                                                </div>
                                            </ListItem>
                                            <Collapse in={openSolutionTags} sx={{ width: '100%', border: `1px solid ${colors.snowywhite}`, borderBottomLeftRadius: "5px", borderBottomRightRadius: "5px" }} timeout="auto" unmountOnExit>
                                                <CollapsibleTree
                                                    data={industryData}
                    ...
                    {(preview) && (
                        ...
                        >
                            <div style={grpstyle}>
                                ...
                                <p style={valueStyle}>{solutionTitle}</p>
                            </div>
                            ...
                            <div style={grpstyle}>
                                ...
                                <p style={{ ...valueStyle, whiteSpace: "pre-wrap" }}>{keyFeatures}</p>
                            </div>
                            <div style={grpstyle}>
                                <p style={titleStyle}>Benefits</p>
                                <p style={{ ...valueStyle, whiteSpace: "pre-wrap" }}>{benifits}</p>
                            </div>
                            {/* <div style={grpstyle}>
                            ...
                            {(solutionLink) && (<div style={grpstyle}>
                                <p style={titleStyle}>Solutions Link</p>
                                <a style={valueStyle} href={solutionLink}
                                    target="_blank"
                            ...
                            <div style={grpstyle}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'row', width: "100%", justifyContent: "flex-start", alignItems: "center", gap: "10px", }}>
                                    <TSISpreadItems items={selectedSolutions || []} />
                    ...
                    >
                        ...
                        >
                            ...
                            <TSIButton
                                ...
                                // disabled={[
                                //     solutionTitle, solutionLink, positioning, industry,
                                //     numofCustomers, whenLaunched, benifits, keyFeatures,
                                //     // collaterals?.length > 0, 
                                //     selectedSolutions?.length > 0, selectedSkills?.length > 0
                                // ].some(field => !field)}
                                variant={'contained'}
                                padding={deviceType == "mobile" ? "5px 15px" : "5px 20px"}
                                load={false}
                                ...
                                handleClick={() => {
                                    const requiredFields = [
                                        ...
                                        numofCustomers, whenLaunched, benifits, keyFeatures,
                                        // collaterals,
                                        selectedSolutions, selectedSkills
                                    ];
                                    ...
                                    preview ? addSolutionsData() : setPreview(true);
                                }}
                            />
                        </div>
                    </div>
...
export default TSIAddSolutionModal
