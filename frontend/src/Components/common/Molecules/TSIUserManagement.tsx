...
const TSIUserManagement = () => {
  ...
  const actions = [
    // { label: "Edit", onClick: (row: any) => { setIsUserEditOpen(true); setIsUserObj(row); console.log("roww",row)} },
  ...
  const columns: any = [
    ...
    // { id: 'mobile', label: 'Mobile', minWidth: 100, format: (value: number) => value.toLocaleString('en-US') },
    { id: 'role', label: 'Role', minWidth: 100, format: (value: number) => value.toLocaleString('en-US') },
    ...
    return (
      <div style={{ display: "flex", flexDirection: 'column', justifyContent: 'flex-start', alignItems: "flex-start", padding: '10px', gap: '10px', width: '100%', height: "100%" }}>
        ...
        {(isUserOpen) && (<TSIAddEditUserModal
          ...
          edit={false}
          refreshUserList={handlegetUserData}
        />)}
...
export default TSIUserManagement
