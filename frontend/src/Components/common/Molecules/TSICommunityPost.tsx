...
const TSICommunityPost = ({ post, iscommunity, isFullText = false, setClickedPost, setPostView, setIsEditPost, onSubmit, isCommentNeeded, isProfile = false }: any) => {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', justifyContent: "flex-start", borderBottom: isComment ? `0px solid ${colors.snowywhite}` : "", gap: '10px', }}>
            ...
            <div
                onClick={() => {
                    ...
                    handlePostNavigation(post, navigation)
                }}
                onMouseEnter={() => { setIsHover(true) }}
                onMouseLeave={() => { setIsHover(false) }}
                style={{ display: 'flex', flexDirection: 'column', width: '100%', justifyContent: "flex-start", borderBottom: `0.5px solid ${colors.snowywhite}`, boxShadow: `20px 20px 50px ${colors.lightmediumSnowyWhite}`, gap: '10px', padding: '10px', borderRadius: '0px', backgroundColor: isHover ? colors.lightPrimarybackground : colors.white, cursor: "pointer" }}>
                        ...
                        <Avatar
                            onClick={(event) => {
                                if (post?.posted_by !== "Anonymous") {
                                    ...
                                    if (post?.content_type == "NEW_ORG") {
                                        navigation(`/community/postdetails?id=${post?.posted_by_account_slug}`);
                                    } else if (post?.content_type == "NEW_PROFESSIONAL") {
                                        navigation(`/talent/postdetails?id=${post?.posted_by_account_slug}`);
                                    } else if (post?.content_type == "NEW_AMBASSADOR") {
                                        navigation(`/ambassador/postdetails?id=${post?.posted_by_account_slug}`);
                                    } else {
                                        navigation(`/community/postdetails?id=${post?.posted_by_account_slug}`);
                                    }
                                }
                ...
                <div style={{ display: 'flex', flexDirection: 'row', justifyContent: "space-between", alignItems: "flex-start", gap: "20px", width: '100%' }}>
                    ...
                    {(post.title) && (<span
                        style={{
                            ...
                            fontFamily: "OpenSans",
                            textTransform: "capitalize",
                            color: isProfile ? colors.graniteGrey : colors.black
                        }}
                ...
                {(post?.image) && (<div onClick={() => { }} style={{ padding: '10px' }}>
                    <img src={post?.image} style={{ maxWidth: "70%", minWidth: '400px', borderRadius: '12px', objectFit: 'contain', cursor: "pointer" }} />
                </div>)}
            </div>
                                ...
                                >
                                    {
                                        discussionData?.map((comment: any, index: number) => {
                                            if (comment?.parent_uuid == "") {
                                                return (
                                                    <>
                                                        <TSIPostComment key={index} comment={comment} reply={true} type={"POST"} discussionData={discussionData} callAgain={() => { getDiscussionData() }} />
                                                    </>
                                                )
                                            }
                                            return null;
...
export default TSICommunityPost
