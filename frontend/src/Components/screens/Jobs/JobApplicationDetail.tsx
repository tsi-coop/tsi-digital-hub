...
import TSIShortlistModal from '../../common/Molecules/TSIShortlistModal';
import TSIRejectionModal from '../../common/Molecules/TSIRejectionModal';
...
const JobApplicationDetail = () => {
    ...
    const [isShortlist, setShortlist] = useState<any>(false);
    ...
    if (!load) {
        return (
            <div style={{ display: 'flex', flexDirection: deviceWidth ? "column" : 'row', justifyContent: deviceWidth ? "flex-start" : "space-between", alignItems: "flex-start", padding: '10px', gap: '20px', backgroundColor: colors.lightPrimary, height: '92%' }}>
                ...
                <div style={{ width: deviceWidth ? "100%" : '100%', height: "100%", backgroundColor: colors.white, borderRadius: '24px', padding: "10px", display: 'flex', flexDirection: 'column', justifyContent: "flex-start", alignItems: "flex-start", overflowY: "scroll", scrollbarWidth: "none" }}>
                    ...
                    <div style={{ width: '100%', display: 'flex', flexDirection: 'row', justifyContent: "space-between", alignItems: "center", padding: '10px', }}>
                        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: "flex-start", alignItems: "center", }}>
                            ...
                            <p style={{
                                ...
                                lineHeight: "19px",
                                textAlign: "left",
                            }}>Job Application - {jobData?.job_title}</p>
                        </div>
                        {(role == "BUSINESS") && (<div style={{ display: 'flex', flexDirection: 'row', justifyContent: "flex-start", alignItems: "center", gap: "10px" }}>
                            <TSIButton
                                name={"Shorlist"}
                                disabled={jobData?.status == "2" || jobData?.status == "3"}
                                variant={'contained'}
                                padding={"5px 20px"}
                                load={false}
                                isCustomColors={true}
                                customOutlineColor={`0px solid ${colors.primary}`}
                                customOutlineColorOnHover={`0px solid ${colors.primary}`}
                    ...
                    <div style={{ width: deviceType == "mobile" ? "100%" : '70%', overflow: 'scroll', scrollbarWidth: 'none', display: 'flex', flexDirection: 'column', justifyContent: "flex-start", alignItems: "flex-start", padding: '15px', paddingTop: "5px", height: "90%" }}>
                        <div
                            style={{
                                display: 'flex',
                                flexDirection: 'column',
                        ...
                        >
                            ...
                            >
                                <div style={{ display: "flex", flexDirection: 'row', justifyContent: 'flex-start', alignItems: "flex-start", padding: '10px', paddingTop: "0px", gap: '10px', paddingLeft: '0px', paddingBottom: "0px" }}>
                                    {
                                        [{ icon: cityB, name: jobData?.city }]?.map((item: any, index: number) => (
                                            <div
                                                key={index}
                                                style={{
                                                    padding: "4px 10px",
                                    ...
                                    {(jobData?.resume_url) && (<div style={{
                                        ...
                                        gap: '10px',
                                        cursor: "pointer"
                                    }} onClick={() => { window.open(jobData?.resume_url) }}>

                                        <InsertLinkIcon sx={{ width: '20px', color: colors.primary }} />{"Resume Link"}
                                    </div>)}
                                </div>
                                {(jobData?.applicant_name) && (<div style={grpstyle}>
                                    <p style={titleStyle}>Application Name</p>
                                    <p style={valueStyle}>{jobData?.applicant_name}</p>
                                </div>)}
                                {(jobData?.job_type) && (<div style={grpstyle}>
                                    <p style={titleStyle}>Job Type</p>
                                    <p style={valueStyle}>{jobData?.job_type}</p>
                                </div>)}
                                {(jobData?.covering_letter) && (<div style={grpstyle}>
                                    <p style={titleStyle}>Cover Letter</p>
                                    <p style={{ ...valueStyle, whiteSpace: "pre-wrap" }}>{jobData?.covering_letter}</p>
                                </div>)}
                                        ...
                                        >
                                            {
                                                discussionData?.map((comment: any, index: number) => {
                                                    if (comment?.parent_uuid == "") {
                                                        return (
                                                            <TSIPostComment key={index} comment={comment} type={"JOB_APPLICATION"} reply={true} discussionData={discussionData} callAgain={() => { getDiscussionData(id) }} />
                                                        )
                                                    }
                                                    return null;
                                                })
                                ...
                                <div style={{ display: 'flex', flexDirection: 'row', width: "100%", justifyContent: "space-between", alignItems: "center", gap: "10px", backgroundColor: colors.lightPrimary, borderRadius: '12px', paddingLeft: '10px', paddingRight: '10px', height: '80px' }}>
                ...
                <TSIShortlistModal
                    ...
                    desc={""}
                    buttonName1={"No"}
                    buttonName2={"Shortlist"}
                    btn2Color={colors.primary}
                    onClose={() => { setShortlist(false) }}
                    onClick={() => { navigation(-1) }}
                />
